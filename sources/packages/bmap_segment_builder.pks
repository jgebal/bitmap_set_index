CREATE OR REPLACE PACKAGE bmap_segment_builder AUTHID CURRENT_USER AS
   ...
   * An element is a SIMPLE_INTEGER value that represents a bitmap.
  ...
  C_ELEMENT_CAPACITY CONSTANT SIMPLE_INTEGER := 30;
  C_SEGMENT_HEIGHT   CONSTANT SIMPLE_INTEGER := &&SEGMENT_HEIGHT__DEF_VAL_IS_3;
  C_SEGMENT_CAPACITY CONSTANT SIMPLE_INTEGER := POWER( C_ELEMENT_CAPACITY, C_SEGMENT_HEIGHT );

  TYPE BIN_INT_LIST          IS TABLE OF SIMPLE_INTEGER INDEX BY SIMPLE_INTEGER;
  TYPE BIN_INT_LIST_LIST     IS TABLE OF BIN_INT_LIST INDEX BY SIMPLE_INTEGER;
  TYPE BMAP_SEGMENT_LEVEL    IS TABLE OF SIMPLE_INTEGER INDEX BY SIMPLE_INTEGER;
  TYPE BMAP_SEGMENT          IS TABLE OF BMAP_SEGMENT_LEVEL INDEX BY SIMPLE_INTEGER;
